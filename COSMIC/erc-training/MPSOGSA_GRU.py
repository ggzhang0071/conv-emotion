...
from train_iemocap import train_or_eval_model
def euclid_dist(x,y):
    temp = 0   
    for i,j in zip(x,y):
        temp += (i-j)**2
...
def MPSOGSA(dataset,max_iters,num_particles,NumSave,lr,resume,savepath):
    ...
    for i in range(num_particles):
        ...
        for k in range(len(Max)):
            p.params.append(np.random.randint(Max[k]*0.5,Max[k]))
        p.params.append(np.random.uniform(1e-5,0.01))
        ...
        for p in particles:
            ...
            for kk,param in enumerate(p.params[:-1]):
                if param<Max[kk]*0.5 or param>Max[kk]:
                    p.params[kk]=np.random.randint(Max*0.5,Max)  
